...
The remote control plane is an independently deployable process. Each Host reuses its daemon token as the Host Secret and only makes one outbound WSS connection to the Relay; the Relay multiplexes Web clients by connection ID and never connects to an inbound macOS port. A seven-day-by-default pairing code and opaque client-facing invite (`/invite/<opaque>/`) are reusable during their sharing window, while each exchange receives a short-lived Ed25519 capability bound to the Host, scope, route, client, and credential generation. Relay persists only invite hashes, credential hashes, generations, and online metadata, so a restart does not invalidate a still-live invite after the Host reconnects. Revoking or rotating a Host credential must disconnect the Host immediately and invalidate old capabilities and invites. The Relay never stores Project/Workspace/Session state, terminal output, or user input. Public deployments must sit behind TLS, enforce a strict Origin, use strong random secrets, and use a persistent data volume.
