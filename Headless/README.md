...
Git requests (`git.panel`, `git.diff`, `git.checkout`, `git.pull`, `git.push`, `git.commit`, and `git.pr.create`) run on a per-Workspace coordinated lane: operations for the same workspace serialize, different workspaces may run concurrently, and only each workspace's queue head claims one bounded auxiliary worker. Responses for different request IDs may therefore complete out of order; correlate them by request ID. A cached `git.panel` stays served while the host revalidates it in a separate bounded stale-while-revalidate lane.
