...
final class TerminalCallbackBridge {
    ...
    /// Synchronous open-url dispatch used by the C action callback so Ghostty
    /// knows the embedder consumed the action and does not fall back to
    /// spawning `/usr/bin/open`. Set on the main actor before any link
    /// activation; the handler must be safe to call from any thread.
    nonisolated(unsafe) var openURLHandler: (@Sendable (String, TerminalOpenURLKind) -> Void)?
    ...
    func handleAction(_ action: ghostty_action_s) {
        ...
        case GHOSTTY_ACTION_MOUSE_SHAPE:
            guard let shape = TerminalMouseShape(action.action.mouse_shape) else {
                TerminalDebugLog.log(
                    .actions,
                    "callback action=mouse_shape unknown raw=\(action.action.mouse_shape.rawValue)"
                )
                break
            }
            TerminalDebugLog.log(
        ...
        case GHOSTTY_ACTION_MOUSE_VISIBILITY:
            ...
            onMouseVisibilityChange?(visible)
        ...
        case GHOSTTY_ACTION_RENDERER_HEALTH:
            ...
            TerminalDebugLog.log(
                .actions,
                "callback action=renderer_health healthy=\(healthy)"
            )
            // NSLog (unlike TerminalDebugLog) reaches the unified log, so the
            // exact moment the renderer trips its "non-functional" failsafe is
            // visible in `idevicesyslog` for diagnosis.
            NSLog("[GhosttyTerminal] renderer_health healthy=\(healthy)")
            (delegate as? any TerminalSurfaceRendererHealthDelegate)?
                .terminalDidChangeRendererHealth(healthy)
