...
test("persisted preset order is normalized", () => {
  ...
  assert.deepEqual(normalizeSessionPresetOrder(null), ["shell", "claude", "codex", "opencode", "trae"]);
});
...
test("loading preset order repairs persisted data", () => {
  ...
  assert.deepEqual(loadSessionPresetOrder(storage, "preset-order"), ["codex", "shell", "claude", "opencode", "trae"]);
  assert.equal(storage.value, '["codex","shell","claude","opencode","trae"]');

  storage.value = "invalid-json";
  assert.deepEqual(loadSessionPresetOrder(storage, "preset-order"), ["shell", "claude", "codex", "opencode", "trae"]);
  assert.equal(storage.value, '["shell","claude","codex","opencode","trae"]');
});

test("preset order controls presentation", () => {
  const presets = orderedSessionPresets(["shell", "codex", "claude"]);

  assert.deepEqual(presets.map(preset => preset.kind), ["shell", "codex", "claude", "opencode", "trae"]);
});

test("preset order moves within bounds", () => {
  const order = ["shell", "claude", "codex", "opencode", "trae"];

  assert.deepEqual(moveSessionPreset(order, "codex", -1), ["shell", "codex", "claude", "opencode", "trae"]);
...
test("Codex, Claude, and OpenCode, plus bound shell overlays, are Agents", () => {
