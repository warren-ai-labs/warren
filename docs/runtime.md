...
1. The desktop and headless daemon start from a clean, host-level baseline.
   ...
   login shell, a valid `SSH_AUTH_SOCK`, and explicit Warren/provider paths and
   settings. It drops terminal integration, task-runner state (`mise`/`direnv`),
   inherited agent IDs, pager/color overrides, proxies, and credentials. The
   detached Ghostline server inherits this baseline and is not rebuilt from a
   launching terminal on every request.
2. Session creation adds Warren's binding variables and the persisted
...
3. The PTY starts the validated login shell (`$SHELL -il`). Its startup files
